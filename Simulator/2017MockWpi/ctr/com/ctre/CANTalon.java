...
public class CANTalon implements MotorSafety, PIDOutput, PIDSource, CANSpeedController
{
    ...
    {
        QuadEncoder(0), AnalogPot(2), AnalogEncoder(3), EncRising(4), EncFalling(5), CtreMagEncoder_Relative(6), CtreMagEncoder_Absolute(
                7), PulseWidth(8);
     ...
     *         1023) on the analog pin of the Talon. The upper 14 bits tracks
     *         the overflows and underflows (continuous sensor).
     */
     ...
     *         using analog sensors, 0 units corresponds to 0V, 1023 units
     *         corresponds to 3.3V When using an analog encoder (wrapping around
     *         1023 to 0 is possible) the units are still 3.3V per 1023 units.
     ...
     *         200 equates to ~0.645 dV per 100ms or 6.451 dV per second. If
     *         this is an analog encoder, that likely means 1.9548 rotations per
     *         sec. For quadrature encoders, each unit corresponds a quadrature
     *         edge (4X). So a 250 count encoder will produce 1000 edge events
     *         per rotation. An example speed of 200 would then equate to 20% of
     *         a rotation per 100ms, or 10 rotations per second.
     */
    ...
    private void applyControlMode(TalonControlMode controlMode)
    {
        ...
        // UsageReporting.report(tResourceType.kResourceType_CANTalonSRX,
        // m_deviceNumber + 1, controlMode.value);
    }
    ...
    public double getP()
    {
        ...
        // throw new
        // IllegalStateException("PID mode only applies in Position and Speed
        // modes.");
        // }
    ...
    double GetNativeUnitsPerRotationScalar(FeedbackDevice devToLookup)
    {
        ...
        { /*
           ...
           * source is the CTRE Mag encoder, then the CPR is known. This is nice
           * in that the calling app does not require knowing the CPR at all. So
           * do both checks here.
           */
     ...
     * 
     * @param targPos
     *            servo position in native Talon units (sensor units).
     * @param targVel
     *            velocity to feed-forward in native Talon units (sensor units
     *            per 100ms).
     * @param profileSlotSelect
     *            which slot to pull PIDF gains from. Currently supports 0 or 1.
     * @param timeDurMs
     *            time in milliseconds of how long to apply this point.
     * @param velOnly
     *            set to nonzero to signal Talon that only the feed-foward
     *            velocity should be used, i.e. do not perform PID on position.
     *            This is equivalent to setting PID gains to zero, but much more
     *            efficient and synchronized to MP.
     * @param isLastPoint
     *            set to nonzero to signal Talon to keep processing this
     *            trajectory point, instead of jumping to the next one when
     *            timeDurMs expires. Otherwise MP executer will eventuall see an
     *            empty buffer after the last point expires, causing it to
     *            assert the IsUnderRun flag. However this may be desired if
     *            calling application nevers wants to terminate the MP.
     * @param zeroPos
     *            set to nonzero to signal Talon to "zero" the selected position
     *            sensor before executing this trajectory point. Typically the
     *            first point should have this set only thus allowing the
     *            remainder of the MP positions to be relative to zero.
    ...
    public boolean pushMotionProfileTrajectory(TrajectoryPoint trajPt)
    {
        ...
        CanTalonJNI.PushMotionProfileTrajectory(m_handle, targPos, targVel, profileSlotSelect, timeDurMs, trajPt.velocityOnly ? 1 : 0,
     ...
     * 
     * @param [out]
     *            motionProfileStatus contains all progress information on the
     ...
     */
    protected void setMotionProfileStatusFromJNI(MotionProfileStatus motionProfileStatus, int flags, int profileSlotSelect, int targPos, int targVel,
            int topBufferRem, int topBufferCnt, int btmBufferCnt, int outputEnable)
    {
